...
function render(){
  ...
  <div class="app-shell">
    ...
    <nav class="mobile-tabs">${['dashboard','schedule','jobs','customers','settings'].map(i=>`<button data-view="${i}" class="${currentView===i?'active':''}"><span>${icons[i]}</span>${cap(i)}</button>`).join('')}</nav>
  </div>`;
  bind();
}
